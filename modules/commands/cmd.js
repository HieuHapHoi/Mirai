...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command.config.envConfig && typeof command.config.envConfig == 'Object') try {
                ...
                logger.loader('Đã tải thành công file' + ' ' + command.config.name);
            } catch (error) {
    ...
    api.sendMessage('Đã tải ' + (moduleList.length - errorList.length) + ' module(s)', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        ...
        logger(`Đã khóa file ${nameModule}!`);
    }

    writeFileSync(configPath, JSON.stringify(configValue, null, 4), 'utf8');
    unlinkSync(configPath + ".temp");

    return api.sendMessage(`Đã khóa ${moduleList.length} module(s)`, threadID, messageID);
}

module.exports.run = function ({ event, args, api }) {
    if (event.senderID != global.config.ADMINBOT) return api.sendMessage(`Bạn không phải admin bot`, event.threadID, event.messageID)
